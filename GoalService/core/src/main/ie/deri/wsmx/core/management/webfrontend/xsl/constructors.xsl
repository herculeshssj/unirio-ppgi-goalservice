...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:param name="head.title">WSMX - MBean Constructor's View</xsl:param>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
               ...
               <tr class="{$classtype}">
                  <td valign="top" class="constructorrow">
                     Constructor for class <xsl:value-of select="@name"/>
                  </td>
               </tr>
               <tr class="{$classtype}">
                  <td>
               ...
               <tr class="{$classtype}">
                  <td valign="bottom" align="right" class="constructorrow">
                     ObjectName:
                     <input type="input" title="Enter something like domain:property=value[,property=value]*" name="objectname"/>
                  </td>
               </tr>
               <tr class="{$classtype}">
                  <td align="right" class="constructorrow">
                     <input type="submit" title="Creates a new instance" value="Create new"/>
   ...
   <xsl:template match="Exception" name="error">
      <xsl:for-each select="Exception">
         <table width="100%" cellpadding="0" cellspacing="0" border="0">
            <tr>
               <td class="fronttab">
                  Exception: <xsl:value-of select="@errorMsg"/>
               </td>
            </tr>
         </table>
      </xsl:for-each>
               ...
               <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <tr>
                     <td width="100%" class="page_title">
                        Constructors for class <xsl:value-of select="@classname"/>>
                     </td>
                  </tr>
               </table>
            </xsl:for-each>
